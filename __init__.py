...
class ArgosTranslateManager:
    ...
    @classmethod
    def autodetect_language(cls, text):
        """
        Rileva la lingua usando langdetect, altrimenti fallback su simple_language_detect.
        """
        if not text or len(text.strip()) < 3:
            return "en"
        try:
            code = detect(text)
            # 'zh-cn', 'zh-tw' ecc. → normalizza a 'zh'
            if code.startswith("zh"):
                code = "zh"
            if code not in cls.LANGUAGE_MAP:
                # Prova mapping per codici tipo 'pt', 'pt-br', 'pt-pt'
                base = code.split('-')[0]
            ...
            # fallback sul sistema grezzo
            return cls.simple_language_detect(text)
    ...
    def ensure_translation_package(cls, source_lang, target_lang):
        ...
        with cls._download_lock:
            ...
            try:
                ...
                print(f"Package size: ~{getattr(target_package, 'size', 0) // 1024 // 1024}MB (this may take a while...)")
    ...
    def translate_text(cls, text, source_lang="auto - Auto-detect", target_lang="en - English"):
        ...
        try:
            ...
            if source_code == "auto":
                detected_lang = cls.autodetect_language(text)
